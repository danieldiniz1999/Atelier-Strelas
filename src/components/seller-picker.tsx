...
/**
 * Botão que abre um popup pedindo para o cliente escolher
 * com qual vendedora quer falar no WhatsApp.
 */
export function SellerPickerButton({
  className,
  ariaLabel = "Falar no WhatsApp",
  message = DEFAULT_WPP_MESSAGE,
  children,
}: Props) {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="mt-2 grid gap-3">
            {SELLERS.map((s) => (
              <a
                key={s.id}
                href={buildWhatsappLink(s.phone, `Olá ${s.name}! ${message}`)}
                target="_blank"
